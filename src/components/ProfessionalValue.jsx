import { motion } from 'framer-motion';
import { Sparkles, Code, UserCheck, Award } from 'lucide-react';

const ProfessionalValue = () => {
  const values = [
    {
      title: "AI Project Generation",
      desc: "real industry use-cases",
      icon: Sparkles
    },
    {
      title: "In-Browser Execution",
      desc: "write/run code",
      icon: Code
    },
    {
      title: "Human Sprint Review",
      desc: "reviewed by senior devs",
      icon: UserCheck
    },
    {
      title: "Verified Certificate",
      desc: "tamper-proof + URL",
      icon: Award
    }
  ];

  return (
    <section className="py-12 md:py-24 px-6 bg-transparent overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="text-center mb-10 md:mb-16"
        >
          <h2 className="text-3xl md:text-5xl font-bold text-white mb-4">
            What you actually get
          </h2>
        </motion.div>

        {/* Desktop Layout (Large Boxes) */}
        <div className="hidden md:grid grid-cols-2 gap-6">
          {values.map((value, index) => (
            <motion.div
              key={index}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: index * 0.1 }}
              whileHover={{ scale: 1.02 }}
              className="p-8 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md transition-all group overflow-hidden relative"
            >
              <div className="relative z-10 flex items-start gap-4">
                <div className="mt-1 p-2 rounded-lg bg-blue-500/10 border border-blue-500/20">
                  <value.icon className="w-6 h-6 text-blue-400" />
                </div>
                <div>
                  <h3 className="text-xl font-bold text-white mb-2">
                    {value.title}
                  </h3>
                  <p className="text-slate-400 font-medium">
                    {value.desc}
                  </p>
                </div>
              </div>
              <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/5 blur-3xl -mr-16 -mt-16 group-hover:bg-blue-500/10 transition-all rounded-full" />
            </motion.div>
          ))}
        </div>

        {/* Mobile Layout (Compact Grid) */}
        <div className="grid md:hidden grid-cols-2 gap-2">
          {values.map((value, index) => (
            <motion.div
              key={index}
              initial={{ opacity: 0, scale: 0.95 }}
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true }}
              transition={{ delay: index * 0.05 }}
              className="bg-white/5 border border-white/10 rounded-lg p-3 flex flex-col items-start gap-1 min-h-[70px] hover:bg-white/10 transition-all duration-300 active:scale-95"
            >
              <value.icon className="w-4 h-4 text-blue-400 mb-1" />
              <h3 className="text-sm font-medium text-white">
                {value.title}
              </h3>
              <p className="text-xs text-gray-400 leading-tight">
                {value.desc}
              </p>
            </motion.div>
          ))}
        </div>
      </div>
    </section>
  );
};

export default ProfessionalValue;

